...
CONFIDENCE_THRESH = 0 # confidence percentage threshold of match points used for homography computation
...
def compute_homography_ransac(matches_a, matches_b):
    ...
    # CONFIDENCE-PRÜFUNG DEAKTIVIERT - Panorama wird immer erstellt
    # if best_confidence_obtained < CONFIDENCE_THRESH:
    #     raise(exceptions.MatchesNotConfident(best_confidence_obtained))
    
    # Stattdessen nur eine Warnung ausgeben
    if best_confidence_obtained < CONFIDENCE_THRESH:
        print(f"⚠️  Warnung: Niedrige Confidence ({best_confidence_obtained}%), aber Panorama wird trotzdem erstellt.")
