...
export class GeometryHelper {
    ...
    async generatePoints() : Promise<THREE.Points | THREE.Mesh>{
        ...
        switch(target) {
            ...
            case 'Data': {
                let generatedWrapper : geometryUtils.GeneratedMeshWrapper
                if (this.useRawData) {
                    const imageJsons = await this.api.getAssets(0, ASSET_TYPE.DATA, this.useLocalAssets)
                    generatedWrapper = geometryUtils.getDataBufferGeometry(new geometryUtils.ScenarioDataWrapper([], imageJsons), this.imgCenterPoints, target)
                } else {
                    let textures : any[] = await this.api.fetchImages(0, ASSET_TYPE.DATA, true);
                    let imageData = await this.textureToImageData(textures)
                    generatedWrapper = geometryUtils.getDataBufferGeometry(new geometryUtils.ScenarioDataWrapper(imageData), this.imgCenterPoints, target)
                }
                geometry = generatedWrapper.geometry
                this.imgMinZ = generatedWrapper.imgMinZ
                this.imgMaxZ = generatedWrapper.imgMaxZ
    ...
    async generatePointsByWorker() : Promise<{mesh: THREE.Points | THREE.Mesh, centerPositions: {[sceneType: string] : Array<THREE.Vector3>}, minZ: number, maxZ: number}>{
        ...
        switch(targetGeometry) {
            ...
            case 'Data': {
                textures = await this.api.fetchImages(0, ASSET_TYPE.DATA, true);
                this.guiWrapper.updateFrameCounter(2)
                break
            }
        }
