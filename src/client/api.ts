...
//my data from data folder 

import louOriginal from "./assets/data/lou.jpg"
import louDepth from "./assets/data/lou.png"

export enum ASSET_TYPE {
    FARM, AI, ACT, KOALA, DATA //
}
...
export class Api {
    ...
    dataAssetsList = [louOriginal, louDepth]
    ...
    async getAssets(steps: number, type: ASSET_TYPE, useLocalBundle = false) : Promise<any[]> {
        ...
        switch(type) {
            ...
            case ASSET_TYPE.DATA: {
                if (!useLocalBundle) {
                    folder = 'data/'
                    imageUrls = ['lou.json', 'loudepth.json']
                } else {
                    imageUrls = this.dataAssetsList
                }
                break
            }
        }
