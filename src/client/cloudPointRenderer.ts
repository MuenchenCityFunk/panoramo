...
import { exportPointCloudData, savePointCloudToFile } from './geometryUtil'
...
export class CloudPointRenderer {
    ...
    exportOptions = {
        export: () => {
            this.exportCurrentPointCloud()
        }
    }

    callbackOptions = {
        renderSelectedMesh: (sceneType: any) => {
            this.renderSelectedMesh(sceneType)
        },
        scaleScene: (scale: number) => {
            this.scaleScene(scale)
        },
    ...
    start() {
        this.guiWrapper = new GuiWrapper(
    ...
    exportCurrentPointCloud() {
        if (this.currentSceneObject && this.currentSceneObject.geometry) {
            const pointCloudData = exportPointCloudData(this.currentSceneObject.geometry);
            const filename = `lou_pointcloud_${new Date().toISOString().slice(0,19).replace(/:/g,'-')}.json`;
            savePointCloudToFile(pointCloudData, filename);
            console.log('Point Cloud exported:', filename);
        } else {
            console.error('No point cloud to export');
        }
    }

    scaleScene(scale: number) {
        if (this.currentSceneObject != undefined) {
            this.currentSceneObject.scale.set( 5, 5, scale );
        }
    }
