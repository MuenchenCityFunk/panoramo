...
export class GuiWrapper {
    ...
    exportButton: GUIController | undefined

    constructor(public interpolationOptions: any, public textureOptions: any, public renderingOptions: any, public parallaxOptions: any, public callbackOptions: any, public exportOptions: any) {
        this.gui = new GUI()
        this.initDefaultGui()
    }
    ...
    initDefaultGui() {
        ...
        this.renderingType = this.sceneSettingsFolder.add(this.renderingOptions, 'type', ['Waifus', 'Farm', 'Koala', 'ai_images', 'Data']).name("Rendering type").onChange((value: string) => {
            this.callbackOptions.renderSelectedMesh(value)
        })
        
        // Export button hinzufügen
        this.exportButton = this.sceneSettingsFolder.add(this.exportOptions, 'export').name("Export Point Cloud").onChange(() => {
            this.callbackOptions.exportCurrentPointCloud()
        })
        
        this.sceneSettingsFolder.add(this.interpolationOptions, 'scale', -2, 2, 0.001).onChange((value: number) => {
    ...
    createTextureOptions() {
        ...
        this.textureRenderFolder.add(this.textureOptions, 'textureUsage').name("Texture as color source").onChange((value: boolean) => {
    ...
    createParallaxOptions() {
        ...
        this.parallaxGui = this.parallaxFolder.add(this.parallaxOptions, 'type', ['None', 'Auto', 'Dynamic']).name("Parallax type").onChange((value: string) => {
    ...
    createDetailedOptions(liteVersion = false) {
        ...
        this.frameOptionGui =  this.optionsFolder.add( this.interpolationOptions, 'frame', 0, 11, 1).onChange((value: number) => {
